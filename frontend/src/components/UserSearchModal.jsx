...
const UserSearchModal = ({ isOpen, onClose, onSelectUser }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        {/* Modal Header */}
          ...
          {!loading &&
            results.map((targetUser) => (
              <div
                ...
                className="p-3 rounded-2xl bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/50 dark:hover:bg-slate-800 border border-slate-200/80 dark:border-slate-700/50 hover:border-indigo-500/50 flex items-center justify-between cursor-pointer transition-all group"
              >
                  ...
                  <div className="relative w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-600/30 border border-indigo-200 dark:border-indigo-500/40 flex items-center justify-center text-indigo-600 dark:text-indigo-300 font-bold overflow-hidden">
                    ...
                    <span
                      className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white dark:border-slate-900 ${
                        targetUser.status === 'online' ? 'bg-emerald-500' : 'bg-slate-400 dark:bg-slate-500'
                      }`}
                    />
                  </div>
                ...
                <span className="text-xs font-semibold px-3 py-1.5 rounded-xl bg-indigo-600/10 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                  Chat
                </span>
              </div>
            ))}
